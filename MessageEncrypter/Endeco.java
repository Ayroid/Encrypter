...
public class Endeco {
    ...
    String symbols = "`1234567890-=~!@ #$%^&*()_+[]\\;',./{}|:\"<>?",
            thirdcode = "F]@#$%MDE0ZaXYwxyzVgb34cijGQR-= [k^&*()C<>?";
    ...
    public String encoder(String input) {
        ...
        return stringReverser(encoded.substring(encoded.length() / 2, encoded.length()))
                + encoded.substring(0, encoded.length() / 2); // Reverse of 2nd half + 1st half of encoded text
    }

    public String decoder(String encoded) {
        String decoded = "";
        if (encoded.length() % 2 == 1) { // rolling back changes made in line 32
            encoded = encoded.substring((encoded.length() / 2) + 1, encoded.length())
                    + stringReverser(encoded.substring(0, (encoded.length() / 2) + 1)); // if string length is odd
        } else {
            encoded = encoded.substring(encoded.length() / 2, encoded.length())
                    + stringReverser(encoded.substring(0, encoded.length() / 2)); // if string length is even
        }
        ...
        switch (ch) {
            ...
            case "D":
                ...
                if ((encoded.length() - 1) % 2 == 0) {
                    turns = Character.getNumericValue(encoded.charAt(encoded.length() - 1)); // Extracting number of
                                                                                             // times message has been
                                                                                             // encoded
                    encoded = encoded.substring(0, encoded.length() - 1); // removing the number of turns from encoded
                                                                          // message
                } else {
                    turns = Character.getNumericValue(encoded.charAt(0)); // Extracting number of times message has been
                                                                          // encoded
                    encoded = encoded.substring(1, encoded.length()); // removing the number of turns from encoded
                                                                      // message
                }
